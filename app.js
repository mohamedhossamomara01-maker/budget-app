...
function CategoryScreen({
  ...
  // تحويش: يخصم من ميزانية محمد بس، مش من ضحي
  const tahwishMonth = entries.filter(e => e.type === "tahwish" && finKey(e.date) === mk && e.id && e.id.startsWith(idPrefix));
  const mTahwish = entryType === "home" ? SUM(tahwishMonth) : 0;
  const mTot = SUM(month),
    tTot = SUM(today),
    rem = budget - mTot - mTahwish;
  ...
  const allMonthEntries = entryType === "home"
    ? [...month, ...tahwishMonth.map(e => ({ ...e, cat: "tahwish", _isTahwish: true }))]
    : [...month];
  ...
  shownEntries.map(e => {
    const isTahwish = e._isTahwish;
    const c = isTahwish ? {ic:"💰",l:"تحويش",c:"#a78bfa"} : catF(categories, e.cat);
    return /*#__PURE__*/React.createElement("div", {
      key:e.id,
...
function SummaryScreen({
  ...
  // تحويش محمد بس يخصم من الأمان، تحويش ضحي بيسجل في الإجمالي بس
  const mTahwishMohy = SUM((entries||[]).filter(e=>e.type==="tahwish"&&e.id&&e.id.startsWith("hn")&&finKey(e.date)===mk));
  ...
  }, [["الثوابت", fixDisplay || 9480, T.red], ["محمد", mHomeTot, T.blue], ["صيانة", mCar, "#a78bfa"], ["💰 تحويش", SUM((entries||[]).filter(e=>e.type==="tahwish"&&e.id&&e.id.startsWith("hn")&&finKey(e.date)===mk)), "#a78bfa"], ["⛽ بنزين", ind ? (ind.petrol||0) : 0, "#f59e0b"], ["🧾 ضريبة", ind ? (ind.tax||0) : 0, "#f59e0b"], ["🔧 نفخ كاوتش", ind ? (ind.tire||0) : 0, "#f59e0b"]].map(([l, v, c]) => /*#__PURE__*/React.createElement("div", {
    key: l,
    style: {
      textAlign: "center"
    }
