...
function CategoryScreen({
  ...
  const all = [...dataSource, ...entries.filter(e => e.type === entryType)].sort((a, b) => b.date.localeCompare(a.date));
...
function SummaryScreen({
  ...
  const prevTotalOut = prevFix + prevDuha + SUM(prevAllH) + SUM(prevAllC) + SUM(prevAllD) + prevIndExp + prevTahwish;
  const prevBalance = Math.max(0, (prevInc + prevIndRev) - prevTotalOut);

  // فلوس قديمة ضحي من الشهر السابق
  const prevDuhaSpent = SUM(prevAllD);
