...
function SummaryScreen({
  ...
  }, [["الثوابت", fixDisplay || 9480, T.red], ["محمد", mHomeTot, T.blue], ["صيانة", mCar, "#a78bfa"], ["💰 تحويش", SUM((entries||[]).filter(e=>e.type==="home"&&e.cat==="saving"&&finKey(e.date)===mk)), "#a78bfa"], ["⛽ بنزين", ind ? (ind.petrol||0) : 0, "#f59e0b"], ["🧾 ضريبة", ind ? (ind.tax||0) : 0, "#f59e0b"], ["🔧 نفخ كاوتش", ind ? (ind.tire||0) : 0, "#f59e0b"]].map(([l, v, c]) => /*#__PURE__*/React.createElement("div", {
    key: l,
    style: {
      textAlign: "center"
    }
...
async function openRouterAnalyze(b64, mime, prompt) {
  // أول: جرب Gemini (مجاني)
  try {
    const txt = await geminiAnalyze(b64, mime, prompt);
    if (txt) return txt;
  } catch(e) {
    console.log("Gemini failed, trying Claude:", e.message);
  }
  // تاني: Claude كـ fallback
  const body = {
...
async function compareBodyPhotos(b64A, b64B, weightA, weightB, mimeA, mimeB) {
  ...
  const body = {
    ...
    messages: [{
      role: "user",
      content: [
        { type: "image", source: { type: "base64", media_type: mimeA || "image/jpeg", data: b64A } },
        { type: "image", source: { type: "base64", media_type: mimeB || "image/jpeg", data: b64B } },
  ...
  const data = await res.json();
  if (data.error) throw new Error(data.error.message);
  return data.content?.[0]?.text || "مش قادر أقارن دلوقتي";
}
