...
async function openRouterAnalyze(b64, mime, prompt) {
  const models = ["google/gemma-4-31b-it:free", "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free", "google/gemma-4-26b-a4b-it:free"];
  for (let mi = 0; mi < models.length; mi++) {
    ...
    try {
      ...
      if (data.error) { clearTimeout(timer); continue; }
      const txt = data.choices?.[0]?.message?.content;
      if (txt) { clearTimeout(timer); return txt; }
    } catch(e) {
      if (e.name !== "AbortError") clearTimeout(timer);
    } finally {
      clearTimeout(timer);
    }
  }
  throw new Error("مش قادر أحلل دلوقتي، جرب تاني");
}
