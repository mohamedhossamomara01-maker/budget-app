...
function SummaryScreen({
  ...
  const prevSaved = monthly[prevMk] || MONTHLY_PRESET[prevMk] || {};
  ...
  const baseInc = n("salary") + n("transport") + n("waste") + (n("old") || autoOld) + n("deals") + n("eid") + n("dohaa") + n("magdy");
  const duhaAllowance = n("home_given") || 0;
  const duhaWSal = n("duha_w_sal") || 0;
  const duhaWSav = n("duha_w_sav") || 0;
  const budget = duhaAllowance + (n("old") > 0 ? 0 : autoDuhaOld);
  const fix = n("car_fixed") + n("rent") + n("internet") + n("charity") + n("mom") + n("ajz") + n("tahwish");
