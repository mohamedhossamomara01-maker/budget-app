...
function CategoryScreen({
  ...
  const inPeriodRange = dateStr => {
    ...
    if (period === "day") return dateStr === DK();
    if (period === "week") {
      const now = new Date();
      const diffToSat = (now.getDay() + 1) % 7; // آخر يوم سبت (بداية الأسبوع المصري)
      const start = new Date(now); start.setHours(0,0,0,0); start.setDate(now.getDate() - diffToSat);
      const end = new Date(start); end.setDate(start.getDate() + 6); end.setHours(23,59,59,999);
      const d = new Date(dateStr + "T12:00:00");
      return d >= start && d <= end;
    }
    if (period === "year") return dateStr.slice(0, 4) === String(new Date().getFullYear());
    return finKey(dateStr) === mk; // month (الافتراضي، زي ما كان قبل كده)
  };
  const periodEntries = useMemo(() => all.filter(e => inPeriodRange(e.date)), [all, period, mk]);
  ...
  /*#__PURE__*/React.createElement("div", {style:{...S.card(), textAlign:"center", margin:"10px 0"}},
    /*#__PURE__*/React.createElement("div", {style:{fontSize:10,color:"#4a6080",marginBottom:3}},
      period==="day" ? "مصاريف النهارده" : period==="week" ? "مصاريف الأسبوع ده" : period==="year" ? "مصاريف السنة دي" : "مصاريف الشهر ده"
    ),
