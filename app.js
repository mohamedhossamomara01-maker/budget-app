...
function CarScreen({
  ...
  const OIL_INTERVAL = 7000;
  const oilEntries = useMemo(() => all.filter(e => e.cat === "oil" && e.km && /^\d+$/.test(String(e.km).trim())), [all]);
  const lastOil = useMemo(() => oilEntries.length ? oilEntries.reduce((a, b) => (+a.km > +b.km ? a : b)) : null, [oilEntries]);
  const [curOdo, setCurOdo] = useState(() => ld("car_odo_v1", 0));
  useEffect(() => sv("car_odo_v1", curOdo), [curOdo]);
  const nextOilKm = lastOil ? +lastOil.km + OIL_INTERVAL : null;
  const kmLeft = nextOilKm !== null && curOdo ? nextOilKm - curOdo : null;
  ...
  return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Tabs, {
    ...
    style: {
      background: kmLeft !== null && kmLeft <= 0 ? "#ef444422" : kmLeft !== null && kmLeft <= 1000 ? "#f59e0b22" : "#0f1a2a",
      border: `1.5px solid ${kmLeft !== null && kmLeft <= 0 ? "#ef444488" : kmLeft !== null && kmLeft <= 1000 ? "#f59e0b88" : "#1a2840"}`,
      borderRadius: 13,
      padding: "11px 13px",
      marginBottom: 10
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: { fontSize: 12, fontWeight: 700, color: "#e2e8f0", marginBottom: 7 }
  }, "🛢️ ميعاد تغيير الزيت الجاي"), !lastOil && /*#__PURE__*/React.createElement("div", {
    style: { fontSize: 11, color: "#4a6080", marginBottom: 7 }
  }, "سجّل عملية زيت بالكيلومتر عشان نقدر نحسبلك الميعاد الجاي"), lastOil && /*#__PURE__*/React.createElement("div", {
    style: { fontSize: 11, color: "#4a6080", marginBottom: 7 }
  }, "آخر تغيير كان عند ", fmt(+lastOil.km), " كم · الهدف عند ", fmt(nextOilKm), " كم (كل ", fmt(OIL_INTERVAL), " كم)"), /*#__PURE__*/React.createElement("div", {
    style: { display: "flex", gap: 7, alignItems: "center", marginBottom: kmLeft !== null ? 8 : 0 }
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    inputMode: "numeric",
    placeholder: "عداد العربية دلوقتي",
    value: curOdo || "",
    onChange: e => setCurOdo(parseFloat(e.target.value) || 0),
    style: { ...S.inp, marginBottom: 0, flex: 1 }
  })), kmLeft !== null && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement(Bar, {
    v: Math.min(curOdo - (+lastOil.km), OIL_INTERVAL),
    max: OIL_INTERVAL,
    c: kmLeft <= 0 ? "#ef4444" : kmLeft <= 1000 ? "#f59e0b" : "#10b981",
    h: 6
  }), /*#__PURE__*/React.createElement("div", {
    style: {
      fontSize: 11,
      fontWeight: 700,
      marginTop: 5,
      color: kmLeft <= 0 ? "#ef4444" : kmLeft <= 1000 ? "#f59e0b" : "#10b981"
    }
  }, kmLeft <= 0 ? `⚠️ متأخر بـ ${fmt(Math.abs(kmLeft))} كم` : `باقي ${fmt(kmLeft)} كم على تغيير الزيت`))), scope === "all" && /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      justifyContent: "space-between",
      alignItems: "center",
    ...
    }, e.date, e.km ? ` · ${e.km} كم` : "", e.note ? ` · ${e.note}` : "", e.paidBy === "doha" ? " · 👩 مرتب ضحي" : ""))), /*#__PURE__*/React.createElement("div", {
      style: {
        display: "flex",
        gap: 7,
        alignItems: "center"
...
function SummaryScreen({
  ...
  // حساب المتبقي من الشهر السابق ونقله تلقائياً
  const prevMk = addM(mk, -1); // الشهر المالي السابق
  const prevPreset = MONTHLY_PRESET[prevMk] || {};
  const prevMonthly = monthly[prevMk] || {};
  // دمج البيانات: بيانات المستخدم بتلغي الـ preset
  const prevSaved = { ...prevPreset, ...prevMonthly };
  // لو المرتب مش موجود في monthly بس موجود في monthly الأحدث استخدم آخر مرتب
  const lastSalary = (() => {
  ...
  const prevCarMohy = SUM(prevAllC.filter(e => e.paidBy !== "doha"));
  ...
  const prevTotalOut = prevFix + prevDuha + SUM(prevAllH) + prevCarMohy + SUM(prevAllD) + prevIndExp + prevTahwish;
  const prevBalance = Math.max(0, (prevInc + prevIndRev) - prevTotalOut);

  // فلوس قديمة ضحي من الشهر السابق
  const prevDuhaSpent = SUM(prevAllD) + prevCarDoha;
  const prevDuhaBalance = Math.max(0, prevDuha - prevDuhaSpent);

  // إضافة المتبقي للشهر الحالي (لو مفيش old مسجل يدوي)
  const autoOld = n("old") > 0 ? 0 : prevBalance;
  const autoDuhaOld = prevDuhaBalance;
  ...
  const budget = duhaAllowance + autoDuhaOld; // مرتب ضحي + متبقي الشهر السابق
  const fix = n("car_fixed") + n("rent") + n("internet") + n("charity") + n("mom") + n("ajz") + n("tahwish");
  ...
  const mCar = SUM(mCarAll.filter(e => e.paidBy !== "doha"));
  const mHomeTot = SUM(mHome);
  const mDuhaTot = SUM(mDuha) + mCarDoha;
  ...
  const totalOut = fix + duhaAllowance + mHomeTot + mCar + (duhaWSal||0) + (duhaWSav||0) + indExpenses + mTahwishMohy;
  ...
  }, fmt(mDuhaTot), " ج")))), (mCar + mCarDoha) > 0 && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
    style: S.sub
  ...
  }, fmt(mCar + mCarDoha), " ج")), mCarDoha > 0 && /*#__PURE__*/React.createElement("div", {
  ...
  }, fmt(mCarDoha), " ج")))), /*#__PURE__*/React.createElement("div", {
    style: {
...
const CLAUDE_KEY = "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";
const GEMINI_KEY = "AQ.Ab8RN6LWpEDzGYWezONCfQy4Jfudamu9FzMOL8EW4aiN1gs4og";
...
function App() {
  ...
  }, tab === "summary" ? "📊 الملخص" : tab === "duha" ? "👩 ضحي" : tab === "food" ? "👨 محمد" : tab === "car" ? "🚗 العربية" : tab === "indrive" ? "🛺 إندرايف" : "🎯 أهدافي"), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 5
  ...
  }, "محمد حسام"))), /*#__PURE__*/React.createElement("div", {
    style: {
      display: "flex",
      gap: 5,
      alignItems: "center"
