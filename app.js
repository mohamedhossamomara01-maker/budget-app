...
const GEMINI_KEY = "AQ.Ab8RN6I3gvB-Ju7VoouFgnxpGXA19-YqbwUMN8zURrQcg8mypA";

async function analyzeBodyPhoto(imageUrl) {
  // تحويل الصورة لـ base64 عشان نبعتها لـ Gemini
  ...
  const body = {
    contents: [{
      parts: [
        { text: "أنت مساعد لياقة بدنية. حلل هذه الصورة وأعطني ملاحظات عن التغيرات الجسمانية المرئية مثل: النحافة في الوجه، منطقة البطن، الجسم عموماً. كن إيجابياً ومشجعاً. الرد بالعربية فقط في 3-4 جمل قصيرة." },
  ...
  return data.candidates?.[0]?.content?.parts?.[0]?.text || "مش قادر أحلل الصورة دلوقتي";
}

async function compareBodyPhotos(urlA, urlB, weightA, weightB) {
  ...
  const [b64A, b64B] = await Promise.all([toBase64(urlA), toBase64(urlB)]);
  const diff = Math.abs(weightA - weightB).toFixed(1);

  const body = {
    contents: [{
      parts: [
        { text: `قارن بين الصورتين دول. الصورة الأولى وزنها ${weightA} كجم والصورة الثانية وزنها ${weightB} كجم، الفرق ${diff} كجم. وضح الفروق المرئية في الجسم بشكل إيجابي ومشجع. الرد بالعربية في 4-5 جمل.` },
  ...
  const data = await gemRes.json();
  return data.candidates?.[0]?.content?.parts?.[0]?.text || "مش قادر أقارن الصور دلوقتي";
}
...
function WeightTracker({ storeKey, startWeight, goalWeight, name, color }) {
  ...
  async function handlePhotoUpload(e, entryDate) {
    ...
    try {
      ...
      setEntries(prev => prev.map(en => en.date === entryDate ? { ...en, photo: url } : en));
        ...
        e.photo && /*#__PURE__*/React.createElement(React.Fragment, null,
          ...
          /*#__PURE__*/React.createElement("button", {
            onClick: async () => {
              ...
              const txt = await analyzeBodyPhoto(e.photo);
              setAnalysisText(txt); setAnalyzing(false);
            },
            ...
            return /*#__PURE__*/React.createElement(React.Fragment, null,
              ...
              /*#__PURE__*/React.createElement("button", {
                onClick: async () => {
                  ...
                  const txt = await compareBodyPhotos(eA.photo, eB.photo, eA.weight, eB.weight);
                  setCompareText(txt); setAnalyzing(false);
                },
