(function() {
  function waitForDeps(callback, tries) {
    tries = tries || 0;
    var reactOk = typeof React !== "undefined" && typeof ReactDOM !== "undefined";
    var supabaseOk = typeof window !== "undefined" && typeof window.supabase !== "undefined";
    if (reactOk && supabaseOk) {
      callback();
    } else if (tries > 200) {
      // React loaded but supabase timed out - run anyway (will work offline)
      if (reactOk) { callback(); }
      else { document.body.innerHTML = "<div style='color:#ef4444;padding:20px;font-family:Cairo,sans-serif;direction:rtl'>⚠️ فشل تحميل التطبيق. تأكد من اتصالك بالإنترنت وأعد تحميل الصفحة.</div>"; }
    } else {
...
async function cloudSave(key, value) {
  if (!sb) {
    console.warn("Supabase not ready - data NOT saved to cloud for key:", key);
    return;
  }
  try {
...
function App() {
  ...
  useEffect(() => {
    if (!sb) {
      console.error("❌ Supabase (sb) is NULL at mount time - cloud sync disabled!");
      return;
    }
    console.log("✅ Supabase ready, loading from cloud...");
    (async () => {
      setSync(true);
